...
(defn test-db []
  (d/as-of (user/db "localhost") #inst "2019-02-01"))
...
(deftest find-max-lunch-order-date-test
  (is (= #inst "2019-02-01"
         (sut/find-max-lunch-order-date (test-db)))))

(deftest make-holiday?-fn-test
  (testing "higher school"
    (let [holiday-ld? (sut/make-holiday?-fn (test-db) true)]
  ...
  (testing "kindergarten"
    (let [holiday-ld? (sut/make-holiday?-fn (test-db))]
...
(deftest find-max-person-paid-period-date-test
  (is (= #inst "2018-06-30"
         (sut/find-max-person-paid-period-date (test-db) Oli)))
  (is (= #inst "2019-04-30"
         (sut/find-max-person-paid-period-date (test-db) Fanda))))
...
(deftest find-school-year-previous-periods-test
  (let [expected-previous-periods [{:db/id 17592186067324,
    ...
    (is (= expected-previous-periods
           (sut/find-school-year-previous-periods (test-db) #inst "2019-01-01")))
    (is (= expected-previous-periods
           (sut/find-school-year-previous-periods (test-db) #inst "2019-01-30")))
    (is (= expected-previous-periods
           (sut/find-school-year-previous-periods (test-db) #inst "2019-02-01")))
    (is (= []
           (sut/find-school-year-previous-periods (test-db) #inst "2020-02-01")))
    (is (= [{:db/id 17592186064383,
             :billing-period/from-yyyymm 201809,
             :billing-period/to-yyyymm 201810}]
           (sut/find-school-year-previous-periods (test-db) #inst "2018-12-31")))))

(deftest find-person-substs-test
  (let [substs (sut/find-person-substs (test-db) 17592186065839)]
