...
(defn- select-lunch-counts [db-spec date]
...
(defn send-lunch-order [db-spec date]
  (let [lunch-counts (not-empty (select-lunch-counts db-spec date))
    ...
    (if-not lunch-counts
      (timbre/info "No lunches for tomorrow. Sending skipped.")
      (try
        (timbre/info "Sending " (:subject msg) "to" (:tos msg))
        (->> (postal/send-message msg)
             (timbre/info "Lunch order sent"))
        (catch Exception e
          (timbre/error "Error sending email" e))))))
