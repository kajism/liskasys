...
(defn hiccup-frame
  [{org-name :-org-name styling :-styling :as user} body]
  (list
   [:head
    [:meta {:charset "UTF-8"}]
    [:meta {:name "viewport" :content "width=device-width, initial-scale=1"}]
    [:title (str "LiškaSys: " org-name)]
    ...
    [:link {:href "https://fonts.googleapis.com/css?family=Roboto+Condensed:400 ,300"
            :rel "stylesheet" :type "text/css"}]
    [:style
     (when (not-empty (:bg1 styling))
       (str "body { background-color: " (:bg1 styling) ";} \n "))
     (when (not-empty (:bg2 styling))
       (str ".table-striped > tbody > tr:nth-of-type(odd), .navbar-default { background-color: " (:bg2 styling) ";}"))]]
   [:body
    body
...
(defn cljs-landing-page
  ([user]
   (hiccup-response
    (hiccup-frame (update user :-org-name #(str "ADMIN " %))
                  [:div
