...
(defn- upload-dir [server-name]
  (or (:upload-dir env) "./uploads/" (get (get-in conf [:datomic :dbs]) server-name) "/"))
...
(defn- ctx [conns server-name]
  (let [conn (conns server-name)]
    {:server-name server-name
     :conn conn}))

(defn api-routes [{conns :datomic}]
  (routes
